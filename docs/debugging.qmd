...
An organization is not getting scraped, and because at least one associated bill is getting scraped, it's breaking the full scrape because it can't find the org for that bill. In the past, this has happened because the organization is not being found by the usual methods, for example, the organization's BodyType is not included in the list of scraped body types. See examples below:
