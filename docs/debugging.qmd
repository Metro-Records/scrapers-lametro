...
- [Recurrent issues](#recurrent-issues)
- Appendices
...
Make sure to search in issues in both la-metro-councilmatic and scrapers-lametro to find possible related issues. To make this organic method of self-documentation more helpful, put searchable error text in your issues and explanations in your PRs :)

Here are some problems that have recurred in the past:

### `cannot resolve pseudo id to Organization:`

An organization is not getting scraped, and because at least one associated bill is getting scraped, it's breaking the full scrape because it can't find the org for that bill. In the past, this has happened because the organization's BodyType is not included in the list of scraped body types. If there are public bills attached to a new body type, we should probably start scraping that body type. 
 - https://github.com/Metro-Records/scrapers-lametro/pull/48
 - https://github.com/Metro-Records/scrapers-lametro/pull/65
